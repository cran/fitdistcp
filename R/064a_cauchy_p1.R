...
#' \deqn{f(x;a,b,\sigma)=\frac{1}{\pi\sigma}\left(1+\left(\frac{x-\mu(a,b)}{\sigma}\right)^2\right)^{-1}}
#' where
#' \eqn{x} is the random variable,
#' \eqn{\mu=a+bt} is the location parameter as a function of parameters \eqn{a,b},
...
#'
#' @example man/examples/example_064_cauchy_p1.R
#'
#' @name cauchy_p1_cp
NULL
#' @rdname cauchy_p1_cp
...
#' @export
#'
qcauchy_p1_cp=function(x,t,t0=NA,n0=NA,p=seq(0.1,0.9,0.1),d1=0.01,d2=0.01,fd3=0.01,
...
#
	debug=TRUE
	debug=FALSE
	if(debug)message("inside qcauchy_p1")
...
#
	...
	opt=optim(c(v1start,v2start,v3start),cauchy_p1_loglik,x=x,t=t,
...
# 4 predictordata
#
	prd=cauchy_p1_predictordata(predictordata,x,t,t0,ml_params)
...
#
	...
	ml_quantiles=qcauchy_p1((1-alpha),t0,ymn=v1hat,slope=v2hat,scale=v3hat)
#
# dmgs
#
...
#
		...
		mu1=cauchy_p1_mu1f(alpha,t0,v1hat,d1,v2hat,d2,v3hat,fd3)
#
# 10 mu2
#
		if(debug)message("calculate mu2")
		mu2=cauchy_p1_mu2f(alpha,t0,v1hat,d1,v2hat,d2,v3hat,fd3)
#
# 11 rhp
#
...
#
		...
		fhat=dcauchy_p1(ml_quantiles,t0,ymn=v1hat,slope=v2hat,scale=v3hat,log=FALSE)
...
# 14 waicscores
#
		waic=cauchy_p1_waic(waicscores,x,t,v1hat,d1,v2hat,d2,v3hat,fd3,
...
# 15 logscores
#
		logscores=cauchy_p1_logscores(logscores,x,t,d1,d2,fd3,aderivs)
...
#
		...
		if(rust){
			rustsim=rcauchy_p1_cp(nrust,x,t=t,t0=t0,rust=TRUE,mlcp=FALSE)
...
#' @inheritParams man
#' @export
rcauchy_p1_cp=function(n,x,t,t0=NA,n0=NA,d1=0.01,d2=0.01,fd3=0.01,rust=FALSE,
...
#
  ...
  t0=t0-meant
	...
	if(mlcp){
		q=qcauchy_p1_cp(x,t,t0=t0,n0=NA,p=runif(n),d1,d2,fd3,aderivs=aderivs)
	...
	if(rust){
		th=tcauchy_p1_cp(n,x,t)$theta_samples
		ru_deviates=numeric(0)
		for (i in 1:n){
			mu=th[i,1]+t0*th[i,2]
			ru_deviates[i]=rcauchy(1,location=mu,scale=th[i,3])
		}
	}
...
#' @inheritParams man
#' @export
dcauchy_p1_cp=function(x,t,t0=NA,n0=NA,y=x,d1=0.01,d2=0.01,fd3=0.01,rust=FALSE,
...
#
  if(centering){
	...
	dd=dcauchy_p1sub(x=x,t=t,y=y,t0=t0,d1,d2,fd3,aderivs=aderivs)
	ru_pdf="rust not selected"
	ml_params=dd$ml_params
	if(rust){
		th=tcauchy_p1_cp(nrust,x,t)$theta_samples
		ru_pdf=numeric(length(y))
		for (ir in 1:nrust){
			mu=th[ir,1]+t0*th[ir,2]
			ru_pdf=ru_pdf+dcauchy(y,location=mu,scale=th[ir,3])
		}
		ru_pdf=ru_pdf/nrust
	}
#
...
#' @inheritParams man
#' @export
pcauchy_p1_cp=function(x,t,t0=NA,n0=NA,y=x,d1=0.01,d2=0.01,fd3=0.01,rust=FALSE,
	nrust=1000,centering=TRUE,debug=FALSE,aderivs=TRUE){
...
#
  if(centering){
	...
	if(rust){
		...
		for (ir in 1:nrust){
			mu=th[ir,1]+t0*th[ir,2]
			ru_cdf=ru_cdf+pcauchy(y,location=mu,scale=th[ir,3])
		}
		ru_cdf=ru_cdf/nrust
	}
#
...
#' @inheritParams man
#' @export
tcauchy_p1_cp=function(n,x,t,d1=0.01,d2=0.01,fd3=0.01,debug=FALSE){
...
#
  ...
  t=t-meant
	...
	th=ru(cauchy_p1_logf,x=x,t=t,n=n,d=3,init=c(0,0,1))
