...
#' \deqn{f(x;\mu,\sigma)=\frac{1}{\pi\sigma}\left(1+\left(\frac{x-\mu}{\sigma}\right)^2\right)^{-1}}
#' where
...
#'
#' @example man/examples/example_042_cauchy.R
#'
#' @name cauchy_cp
NULL
#' @rdname cauchy_cp
...
#' @export
#'
qcauchy_cp=function(x,p=seq(0.1,0.9,0.1),d1=0.01,fd2=0.01,
...
# 2 ml param estimate
#
	opt=optim(c(mean(x),sd(x)),cauchy_loglik,x=x,control=list(fnscale=-1))
	v1hat=opt$par[1]
	v2hat=opt$par[2]
...
#
	ml_quantiles=qcauchy((1-alpha),location=v1hat,scale=v2hat)
#
# dmgs
#
...
# 12 waicscores
#
		waic=cauchy_waic(waicscores,x,v1hat,d1,v2hat,fd2,lddi,lddd,lambdad_rhp,aderivs)
		waic1=waic$waic1
		waic2=waic$waic2
#
# 13 logscores
#
		logscores=cauchy_logscores(logscores,x,d1,fd2,aderivs)
...
#
		...
		if(rust){
			rustsim=rcauchy_cp(nrust,x,rust=TRUE,mlcp=FALSE)
...
#' @inheritParams man
#' @export
rcauchy_cp=function(n,x,d1=0.01,fd2=0.01,rust=FALSE,mlcp=TRUE,
	debug=FALSE,aderivs=TRUE){
...
#	stopifnot(is.finite(n),!is.na(n),is.finite(x),!is.na(x))
	...
	if(mlcp){
		q=qcauchy_cp(x,runif(n),d1=d1,fd2=fd2,aderivs=aderivs)
	...
	if(rust){
		th=tcauchy_cp(n,x)$theta_samples
		ru_deviates=numeric(0)
		for (i in 1:n){
			ru_deviates[i]=rcauchy(1,location=th[i,1],scale=th[i,2])
		}
	}
...
#' @inheritParams man
#' @export
dcauchy_cp=function(x,y=x,d1=0.01,fd2=0.01,rust=FALSE,nrust=1000,
	debug=FALSE,aderivs=TRUE){

	stopifnot(is.finite(x),!is.na(x),is.finite(y),!is.na(y))

	dd=dcauchysub(x=x,y=y,d1,fd2,aderivs=aderivs)
	ru_pdf="rust not selected"
	if(rust){
		th=tcauchy_cp(nrust,x)$theta_samples
		ru_pdf=numeric(length(y))
		for (ir in 1:nrust){
			ru_pdf=ru_pdf+dcauchy(y,location=th[ir,1],scale=th[ir,2])
		}
		ru_pdf=ru_pdf/nrust
	}
...
#' @inheritParams man
#' @export
pcauchy_cp=function(x,y=x,d1=0.01,fd2=0.01,rust=FALSE,nrust=1000,
	debug=FALSE,aderivs=TRUE){
	...
	if(rust){
		...
		for (ir in 1:nrust){
			ru_cdf=ru_cdf+pcauchy(y,location=th[ir,1],scale=th[ir,2])
		}
		ru_cdf=ru_cdf/nrust
	}
		op=list(	ml_params=dd$ml_params,
...
#' @inheritParams man
#' @export
tcauchy_cp=function(n,x,d1=0.01,fd2=0.01,debug=FALSE){

#	stopifnot(is.finite(n),!is.na(n),is.finite(x),!is.na(x))
	stopifnot(is.finite(x),!is.na(x))

	t=ru(cauchy_logf,x=x,n=n,d=2,init=c(mean(x),sd(x)))
