...
#' \deqn{F(x;\mu,\sigma)=\exp\left(-\exp\left(-\frac{x-\mu}{\sigma}\right)\right)}
#' where
...
#'
#' @example man/examples/example_050_gumbel.R
#'
#' @name gumbel_cp
NULL
#' @rdname gumbel_cp
...
#
	ml_quantiles=qgumbel((1-alpha),mu=v1hat,sigma=v2hat)
#
# dmgs
#
...
#
		...
		fhat=dgumbel(ml_quantiles,mu=v1hat,sigma=v2hat)
...
#
		...
		if(rust){
			rustsim=rgumbel_cp(nrust,x,rust=TRUE,mlcp=FALSE)
...
#	stopifnot(is.finite(n),!is.na(n),is.finite(x),!is.na(x))
	...
	if(rust){
		...
		for (i in 1:n){
			ru_deviates[i]=rgumbel(1,mu=th[i,1],sigma=th[i,2])
		}
	}
...
dgumbel_cp=function(x,y=x,rust=FALSE,nrust=1000,
	...
	if(rust){
		...
		for (ir in 1:nrust){
			ru_pdf=ru_pdf+dgumbel(y,mu=th[ir,1],sigma=th[ir,2])
		}
		ru_pdf=ru_pdf/nrust
	}
...
pgumbel_cp=function(x,y=x,rust=FALSE,nrust=1000,
	...
	if(rust){
		...
		for (ir in 1:nrust){
			ru_cdf=ru_cdf+pgumbel(y,mu=th[ir,1],sigma=th[ir,2])
		}
		ru_cdf=ru_cdf/nrust
	}
	op=list(	ml_params=dd$ml_params,
...
#	stopifnot(is.finite(n),!is.na(n),is.finite(x),!is.na(x))
	...
	t=ru(gumbel_logf,x=x,n=n,d=2,init=c(mean(x),sd(x)))
