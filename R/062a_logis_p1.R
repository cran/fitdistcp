...
#' \deqn{f(x;a,b,\sigma)=\frac{1}{1+e^{-(x-\mu(a,b))/\sigma}}}
#' where
#' \eqn{x} is the random variable,
#' \eqn{\mu=a+bt} is the location paramter, and \eqn{\sigma>0} is
#' the scale parameter.
#'
...
#'
#' @example man/examples/example_062_logis_p1.R
#'
#' @name logis_p1_cp
NULL
#' @rdname logis_p1_cp
...
#
	...
	opt=optim(c(v1start,v2start,v3start),logis_p1_loglik,x=x,t=t,
...
# 4 predictordata
#
	prd=logis_p1_predictordata(predictordata,x,t,t0,ml_params)
...
#
		...
		if(rust){
			rustsim=rlogis_p1_cp(nrust,x,t=t,t0=t0,rust=TRUE,mlcp=FALSE)
...
#
  ...
  t0=t0-meant
	...
	if(rust){
		th=tlogis_p1_cp(n,x,t)$theta_samples
		ru_deviates=numeric(0)
		for (i in 1:n){
			mu=th[i,1]+t0*th[i,2]
			ru_deviates[i]=rlogis(1,location=mu,scale=th[i,3])
		}
	}
...
#
  if(centering){
	...
	if(rust){
		th=tlogis_p1_cp(nrust,x,t)$theta_samples
		ru_pdf=numeric(length(y))
		for (ir in 1:nrust){
			mu=th[ir,1]+t0*th[ir,2]
			ru_pdf=ru_pdf+dlogis(y,location=mu,scale=th[ir,3])
		}
		ru_pdf=ru_pdf/nrust
	}
#
...
#
  if(centering){
	...
	if(rust){
		...
		for (ir in 1:nrust){
			mu=th[ir,1]+t0*th[ir,2]
			ru_cdf=ru_cdf+plogis(y,location=mu,scale=th[ir,3])
		}
		ru_cdf=ru_cdf/nrust
	}
#
...
#
  ...
  t=t-meant
	...
	th=ru(logis_p1_logf,x=x,t=t,n=n,d=3,init=c(0,0,1))
